<script lang="ts">
	...
	const { label, options, defaultSelected, onChange, disabled }: MultiSelectProps = $props();
	...
	const removeOption = (index: number, value: string) => {
		const newSelectedOptions = selected.filter((opt) => opt !== value);
		selected = newSelectedOptions;
		if (onChange) onChange(newSelectedOptions);
	};
...
<div class="w-full">
	...
	<div class="relative z-20 inline-block w-full">
		<div class="relative flex flex-col items-center">
			...
			<div onclick={toggleDropdown} class="w-full">
				<div
					class="mb-2 flex h-11 rounded-lg border border-gray-300 py-1.5 pr-3 pl-3 shadow-theme-xs outline-hidden transition focus:border-brand-300 focus:shadow-focus-ring dark:border-gray-700 dark:bg-gray-900 dark:focus:border-brand-300"
				>
					<div class="flex flex-auto flex-wrap gap-2">
						{#if selectedValuesText.length > 0}
							{#each selectedValuesText as text, index (text)}
								<div
								...
								>
									...
									<div class="flex flex-auto flex-row-reverse">
										<div
											onclick={() => removeOption(index, text)}
					...
					<div class="flex w-7 items-center py-1 pr-1 pl-1">
						...
						>
							<svg
								class={`stroke-current ${isOpen ? 'rotate-180' : ''}`}
								width="20"
								height="20"
								viewBox="0 0 20 20"
								fill="none"
